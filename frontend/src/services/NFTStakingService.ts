    ...
    try{
        ...
        if(nftIds.length > 0) {
            ...
            for(let i = offset; i < offset + paginationLimit; i++) {
                ...
                if(parseInt(nftIds[i]) > 20) continue;
                let tokenUri = '';
...
export const stakeToken = async (chainId: number, ownerAddress: string, mintedTokeIds: Array<string>, commonNFT: number, rareNFT: number, queenNFT: number, hyperNFT: number, node: boolean) => {    
    ...
    try{
        ...
        //     tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
        // })    
        // await contractMinter.methods.safeTransferFrom(ownerAddress, '0xc368918959611A2f2A793060a127Aa1054Fe82B0', 1).send({ from: ownerAddress }, (_error: Error, tx: string) => {
        //     tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
        // })
        // await contractMinter.methods.safeTransferFrom(ownerAddress, '0xc368918959611A2f2A793060a127Aa1054Fe82B0', 2).send({ from: ownerAddress }, (_error: Error, tx: string) => {
        //     tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
        // })
        // await contractMinter.methods.setBaseURI('https://ipfs.io/ipfs/QmQf4nSSnm4ihuZ2ThMwpTEytYp66RDmK2MVbxjCn372Jm/').send({ from: ownerAddress }, (_error: Error, tx: string) => {
        //     tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
        // })
        // for(let i=0; i< 10; i++)
        // {
        //     await contractMinter.methods.mint().send({ from: ownerAddress, value: '10000' }, (_error: Error, tx: string) => {
        //      tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
        //      })  
        // } 
        for(let i=0; i<mintedTokeIds.length; i++)
        {
            console.log(mintedTokeIds[i])
            await contractMinter.methods.safeTransferFrom(ownerAddress, '0xc368918959611A2f2A793060a127Aa1054Fe82B0', mintedTokeIds[i]).send({ from: ownerAddress }, (_error: Error, tx: string) => {
                tx ? handleTransaction(tx, TransactionType.mint) : clearTransaction()
            }) 
        }
        return 1;
